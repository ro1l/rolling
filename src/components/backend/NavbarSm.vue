  ...
  <div class="d-lg-none">
    <div class="row">
      <div class="col-12 py-3 fixed-bottom d-flex justify-content-end z-4">
        <button class="btn btn-dark rounded-0 me-4
      me-1" type="button" data-bs-toggle="collapse"
      data-bs-target="#collapseExample" aria-expanded="false"
          aria-controls="collapseExample" @click="toggleOffcanvas">
          <i class="bi bi-list fs-2" v-if="isMenuOpen === false"></i>
          <i class="bi bi-x-lg fs-2" v-if="isMenuOpen === true"></i>
        </button>
      </div>
    </div>
    <div class="collapse z-1 fixed-top" id="collapseExample" :class="{ show: isMenuOpen }">
      <div class="container vh-100 w-100 p-5 bg-dark">
        <ul class="navbar-nav text-black">
          ...
          border-white">Rolling</router-link>
          </li>
          <li class="nav-item mb-4">
            <router-link class="nav-link text-white d-flex align-items-center
        fs-5 p-0 m-0 font-family-taipei" :to="{ name: '產品' }">
              產品</router-link>
          </li>
          <li class="nav-item mb-4">
            <router-link class="nav-link text-white d-flex align-items-center
