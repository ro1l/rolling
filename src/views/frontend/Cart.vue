<template>
  ...
  <div class="cart-box">
    <div class="cart" v-if="cartProducts.total !== 0">
      <div class="cart-box">
        <div class="coupon">
          <label for="coupon" class="text-deep">優惠代碼</label>
          <input type="text" name="" id="coupon" placeholder="請輸入優惠券代碼" v-model="couponCode"
            @change="addCouponCode(couponCode)">
        </div>
        <div class="cart-item" v-for="item in cartProducts.carts" :key="item.id">
          <div class="price-sm text-deep">
            <a href="" class="text-deep" @click.prevent="delProduct(item.id)"
          ...
          <div class="content text-deep">
            ...
            <div class="control">
              <button @click.prevent="updateCart(item, item.qty - 1)" :disabled="item.qty === 1"><i
                  class="bi bi-dash"></i></button>
              <input type="number" name="" id="qty" min="1"
              v-model.number="item.qty" @change="updateCart(item, item.qty)"
                :disabled="item.id === cartLoadingItem.loadingItem">
              <button @click.prevent="updateCart(item, item.qty + 1)">
                <i class="bi bi-plus"></i></button>
            </div>
            <small v-if="item.coupon"><em>已套用{{ item.coupon.code }}優惠券</em></small>
          </div>
        ...
        <div class="total">
          ...
          <p v-if="cartProducts.total !== cartProducts.final_total">折扣
            <span><i class="bi bi-dash"></i>NT${{ $filters.currency
              (cartProducts.total - cartProducts.final_total) }}</span>
          </p>
    ...
    <div class="check-box bg-color" v-if="cartProducts.total !== 0">
      <router-link class="border-btn text-deep" :to="{ name: '訂單填寫' }">訪客結帳</router-link>
    </div>
  </div>
